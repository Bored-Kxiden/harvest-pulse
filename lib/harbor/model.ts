export type Resolution = 'called' | 'reacted' | 'proposed_later' | 'message' | 'dismissed'
export type Moment = { id: string; at: string; person: string; kind: Resolution; text: string; source: 'manual' | 'walking_stop' | 'dispatch'; proposedTime?: string; feedback?: 'good' | 'bad'; reminderDone?: boolean; cueId?: string }
...
export function parseState(raw: string): HarborState | null {
 try {
  ...
  if(!Array.isArray(s.moments)||!s.moments.every(m=>m && typeof m.id==='string' && typeof m.text==='string' && typeof m.person==='string' && Number.isFinite(Date.parse(m.at)) && ['called','reacted','proposed_later','message','dismissed'].includes(m.kind))) return null
